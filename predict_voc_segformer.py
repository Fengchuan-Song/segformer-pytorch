...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict SegFormer semantic segmentation masks in VOC format.")
    parser.add_argument("--model_path", type=str, default="/data/SegFormer/weights/best_epoch_weights_ep099_mIoU0.748.pth", help="Path to trained .pth weights.")
    ...
    parser.add_argument("--output_dir", type=str, default="/data/SegFormer/predict_results/SegmentationClass", help="VOC-style output directory.")
    parser.add_argument("--num_classes", type=int, default=9, help="Number of classes, including background.")
    parser.add_argument("--phi", type=str, default="b0", choices=["b0", "b1", "b2", "b3", "b4", "b5"], help="SegFormer backbone scale.")
...
def collect_image_paths(input_path, dataset_path=None, image_dir="JPEGImages"):
...
def main():
    ...
    img_dir = output_dir.parent / "JPEGImages"
    if args.copy_images:
        img_dir.mkdir(parents=True, exist_ok=True)

    model = load_model(args.model_path, args.num_classes, args.phi, device)
